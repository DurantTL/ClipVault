...
@MainActor final class LibraryViewModel: ObservableObject {
    @Published var project: ClipVaultProject; @Published var selectedClipID: UUID?; @Published var filter: String = "All Clips"; @Published var previewClip: Clip?; let store = ProjectStore(); let mover = FileMoveService(); let security = SecurityScopedBookmarkManager()
    ...
    func moveSelected(to folder: String) { guard let id = selectedClipID, let i = project.clips.firstIndex(where: {$0.id == id}) else { return }; do { try mover.move(clip: &project.clips[i], to: folder, projectFolder: security.projectFolderURL(for: project)); save() } catch { project.clips[i].errorMessage = error.localizedDescription } }
