        ...
        try await Task.detached(priority: .utility) { [security] in
            try security.withAccess(to: source) {
                try security.withAccess(to: destination) {
                    ...
                    if mode == .strong {
                        guard try self.sha256(source) == self.sha256(destination) else { throw NSError(domain: "ClipVault", code: 2, userInfo: [NSLocalizedDescriptionKey: "SHA256 checksum mismatch."]) }
                    }
                }
            }
        }.value
