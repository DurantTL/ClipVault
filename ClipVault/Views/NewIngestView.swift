...
struct NewIngestView: View {
      ...
      VStack(alignment: .leading, spacing: 10) {
        ...
        ScrollView(.horizontal, showsIndicators: false) {
          HStack(spacing: 8) {
            ...
            Button("Reload") {
              preflight.reset()
              // Keep prefersNewOnlySelection across rescan; Preflight refresh re-applies.
  ...
  /// Select New Only is gated on PreflightMediaCheck statuses.
  /// If results are missing, run Preflight first, then select `.newMedia` only.
  private func selectNewOnly() {
    vm.prefersNewOnlySelection = true
    if preflight.hasResults {
    ...
    Task {
      await preflight.run(ingest: vm, settings: settings)
      // run() re-applies when prefersNewOnlySelection / skipAlreadyCopied is set
    }
  }
